...
library(readr) # Needed to read in data
...
colnames(data) <- c("Region", "House_price", "Population", "Crime", "Average_age", "Household_income")
#write.csv(data, "Mult_regression.csv")
# Check the correlation structure

cor(data)

#data <- read_csv("Mult_regression.csv")
...
ggplot(data, aes(x=House_price, y=Population)) + geom_point() + geom_smooth(method="lm")
ggplot(data, aes(x=House_price, y=Crime)) + geom_point() + geom_smooth(method="lm")
ggplot(data, aes(x=House_price, y=Average_age)) + geom_point() + geom_smooth(method="lm")
ggplot(data, aes(x=House_price, y=Household_income)) + geom_point() + geom_smooth(method="lm")

rcorr(data$House_price, data$Population)
rcorr(data$House_price, data$Crime)
rcorr(data$House_price, data$Average_age)
rcorr(data$House_price, data$Household_income)

# First let's build a null model
model0 <- lm (House_price ~ 1, data=data)

# First let's build a model with all predictors
model1 <- lm (House_price ~ Population + Crime + Average_age + Household_income, data=data)
...
leapsmodels <- regsubsets (House_price ~ Population + Crime + Average_age + Household_income, data=data)
