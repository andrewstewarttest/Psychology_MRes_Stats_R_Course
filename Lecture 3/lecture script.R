...
data %>% separate (col="Condition", into=c("Prime", "Target"), sep="_") %>% 
...
ggplot (data_summ, aes (x=Condition, y=Mean, group=Condition, fill=Condition, ymin=Mean-sd, ymax=Mean+sd)) + 
  geom_bar(stat = "identity", width=.5) + geom_errorbar(width=.25) +  
...
# Violin Plot
ggplot (data_long, aes (x=Condition, y=RT, group=Condition, fill=Condition)) + geom_violin() + 
  geom_jitter(alpha=.25, position=position_jitter(0.05)) + guides(colour=FALSE, fill=FALSE) +
...
ggplot(mpg, aes(displ, hwy, colour = class)) + 
  ...
  guides(colour=FALSE) + xlab("Displacement (litres)") + ylab("Highway Fuel Consumption (mpg)") +
...
ggplot(mpg, aes(displ, hwy, colour=class)) + geom_jitter(width=0.05, alpha=.5, size=4) + 
  xlab("Displacement (litres)") + ylab("Highway Fuel Consumption (mpg)") +
  ggtitle ("Scatter Plot of Highway Fuel Consumption against \nEngine Displacement Grouped by Class")

ggplot(mpg, aes(displ, hwy, colour=cyl)) + geom_jitter(width=0.05, alpha=.5, size=4) + 
  xlab("Displacement (litres)") + ylab("Highway Fuel Consumption (mpg)") +
...
# first a static plot
data <- economics.tidy
ggplot(data, aes(mean_pop, mean_un)) + ylim(-3, 3) + xlim (-3, 3) +
  geom_point(size=5, colour="red")+ ggtitle("All Years") + labs (x="Population Size (Z-score)", 
                                                                 y="Unemployment Size (Z-score)")
...
ggplot(data, aes(mean_pop, mean_un)) + ylim(-3, 3) + xlim (-3, 3) +
  geom_point(size=5, colour="red") + labs (title = "Year: {as.integer(frame_time)}", x="Population (Z-score)", y="Unemployment (Z-score)") +
  transition_time(year) + theme(title=element_text(size=15)) +
  ease_aes('linear')
...
ggplot(df_Americas, aes(x = lifeExp, y = fct_reorder(country, lifeExp))) +
  ...
  scale_x_continuous(name = "life expectancy (years)",
                     limits = c(30, 85),
                     expand = c(0, 0)) +
  scale_y_discrete(name = NULL, expand = c(0, 0.5)) + facet_wrap(~year) + 
...
ggplot(df_Americas, aes(x = lifeExp, y = fct_reorder(country, lifeExp))) +
  ...
  labs(title = 'Year: {frame_time}') + theme(title=element_text(size=15)) +
  transition_time(year) +
  ease_aes('linear')
