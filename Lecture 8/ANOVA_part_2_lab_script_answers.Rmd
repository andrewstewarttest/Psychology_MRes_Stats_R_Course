...
First we need to load our packages - tidyverse, afex, and psych.
...
The datafile is called ANOVA_lab2_data1.csv. We  need to load our dataset and map it onto a variable we're calling 'data'.

```{r, message=FALSE}
data <- read_csv("ANOVA_lab2_data1.csv")
```
...
Let's set our three IV columns as factors and look at our data again - note the three columns are now recognised as factors.

```{r}
data$Difficulty <- as.factor(data$Difficulty)
data$Time_Pressure <- as.factor(data$Time_Pressure)
data$Group <- as.factor(data$Group)
data
```

We are first going to visualise the whole dataset (ignoring the Group factor initially). We will construct a boxplot first. 

```{r}
ggplot(data, aes(x = Difficulty:Time_Pressure, y = RT, fill = Difficulty:Time_Pressure)) + 
  geom_boxplot() +
  guides(fill = FALSE) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) + 
  scale_x_discrete(labels = c("Easy\nNo Time Pressure","Easy\nTime Pressure", "Hard\nNo Time Pressure", "Hard\nTime Pressure")) + 
  labs(x = "", y = "RT (ms.)", title = "Boxplot of Reaction Time to Easy vs. Hard Maths Problems Under\nTime Pressure vs. No Time Pressure") 
```
...
```{r}
ggplot(data, aes(x = Difficulty:Time_Pressure, y = RT, fill = Difficulty:Time_Pressure)) + 
  stat_summary(fun.data = mean_cl_boot) + geom_jitter(aes(colour = Group), width = .2, alpha = .5) +
  guides(fill = FALSE) + scale_color_brewer(palette = "Dark2") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) + 
  scale_x_discrete(labels = c("Easy\nNo Time Pressure","Easy\nTime Pressure","Hard\nNo Time Pressure", "Hard\nTime Pressure")) + 
  labs(x = "", y = "RT (ms.)", title = "Plot of Reaction Time Means and 95% CIs to Easy vs. Hard Maths Problems Under\nTime Pressure vs. No Time Pressure") 
...
ggplot(data, aes(x = Difficulty:Time_Pressure, y = RT, fill = Difficulty:Time_Pressure)) + 
  ...
  stat_summary(fun.data = mean_cl_boot) + guides(fill = FALSE) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) + 
  scale_x_discrete(labels = c("Easy\nNo Time Pressure","Easy\nTime Pressure","Hard\nNo Time Pressure", "Hard\nTime Pressure")) + 
  labs(x = "", y = "RT (ms.)", title = "Reaction Time to Easy vs. Hard Maths Problems Under\nTime Pressure vs. No Time Pressure") + 
  coord_flip() 
```
...
ggplot(data, aes(x = Difficulty:Time_Pressure, y = RT, fill = Difficulty:Time_Pressure)) + 
  facet_wrap(~ Group) +
  geom_boxplot() + geom_jitter(width = .1, alpha = .2) + 
  guides(fill = FALSE) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) + 
  scale_x_discrete(labels = c("Easy\nNo Time Pressure","Easy\nTime Pressure","Hard\nNo Time Pressure", "Hard\nTime Pressure")) + 
  labs(x = "", y = "RT (ms.)", title = "Boxplot of Reaction Time to Easy vs. Hard Maths Problems Under\nTime Pressure vs. No Time Pressure") 
...
We are now going to generate some summary descriptives using the psych and tidyverse methods.

```{r}
describeBy(data$RT, group = list(data$Difficulty, data$Time_Pressure, data$Group))
data %>% group_by(Group, Difficulty, Time_Pressure) %>% summarise(mean(RT), sd(RT))
```
...
```{r}
model <- aov_4(RT ~ Difficulty * Time_Pressure * Group + (1 + Difficulty * Time_Pressure | Subject), data)
anova(model)
```
...
```{r}
data_arts <- filter(data, Group == "Arts_Students")
data_maths <- filter(data, Group == "Maths_Students")
data_psych <- filter(data, Group == "Psychology_Students")
```

We now run separate ANOVAs - one for each Group.  First the Arts_Students.

```{r}
model_arts <- aov_4(RT ~ Difficulty * Time_Pressure + (1 + Difficulty * Time_Pressure | Subject), data_arts)
anova(model_arts)
```
...
```{r}
model_maths <- aov_4(RT ~ Difficulty * Time_Pressure + (1 + Difficulty * Time_Pressure | Subject), data_maths)
anova(model_maths)
```
...
```{r}
model_psych <- aov_4(RT ~ Difficulty * Time_Pressure + (1 + Difficulty * Time_Pressure | Subject), data_psych)
anova(model_psych)
```
...
ggplot(data_psych, aes(x = Difficulty:Time_Pressure, y = RT, fill = Difficulty:Time_Pressure)) + 
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) + 
  scale_x_discrete(labels = c("Easy\nNo Time Pressure","Easy\nTime Pressure","Hard\nNo Time Pressure", "Hard\nTime Pressure")) + 
  labs(x = "", y = "RT (ms.)", title = "Reaction Time of Psychology Students to Easy vs. Hard\nMaths Problems Under Time Pressure vs. No Time Pressure") + 
  coord_flip() 
```
